...
// Returns the sum of all the elements in the iterator.
func SumCount[T Number](it iter.Seq[T]) (T, int) {
	var count int
	return Fold(it, 0, func(a, b T) T {
		count++
		return a + b
...
func Product[T Number](it iter.Seq[T]) T {
...
func Average[T Number](it iter.Seq[T]) T {
	return Fold(Enumerate(it), *new(T), func(result T, item Pair[int, T]) T {
...
func Mean[T Number](seq iter.Seq[T]) float64 {
	var sum float64
	var count int
	for value := range seq {
		sum += float64(value)
